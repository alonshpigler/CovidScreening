...
# from https://github.com/Yuol96/pytorch-triplet-loss/blob/master/model/triplet_loss.py
...
def pairwise_distances(embeddings, squared=False):
	...
	clamp_distances = distances.clamp(min=0)
	# clamp_distances = torch.relu(distances)

	if not squared:
		epsilon=1e-16
		mask = torch.eq(clamp_distances, 0).float()
		clamp_distances += mask * epsilon
		square_distances = torch.sqrt(clamp_distances)
		clamp_distances = square_distances * (1-mask)

	return clamp_distances
...
def batch_all_triplet_loss(labels, embeddings, margin, squared=False,opposite=True,loss_type='triplet_plate'):
	"""
	...
	if loss_type == 'triplet_plate':
		triplet_loss = direction * anchor_positive_dist + margin
	elif loss_type == 'contractive_plate':
		triplet_loss = direction * (anchor_positive_dist - anchor_negative_dist) + margin
	else:
